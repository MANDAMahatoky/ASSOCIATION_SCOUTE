...
$membre       = $db->query("SELECT * FROM mpikambana WHERE id_membre = $id")->fetch();
...
if (!$membre) { header('Location: liste.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '' || $prenom === '' || $type_membre === '') {
        ...
        $stmt = $db->prepare("
            UPDATE mpikambana SET
                nom = ?, prenom = ?, type_membre = ?, date_naissance = ?,
                adresse = ?, telephone = ?, email = ?, code_membre = ?,
                fiantso = ?, id_branche = ?, id_fivondronana = ?
            WHERE id_membre = ?
        ");
        $stmt->execute([
            $nom, $prenom, $type_membre, $date_naissance,
            $adresse, $telephone, $email, $code_membre,
            $fiantso, $id_branche, $id_fivondronana, $id
        ]);
...
// Valeurs à afficher (POST si erreur, sinon BD)
$v = $_SERVER['REQUEST_METHOD'] === 'POST' ? $_POST : $membre;
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Modifier Membre — Association Scoute</title>
...
<div class="container" style="max-width:700px;">
    ...
    <div style="margin-bottom:24px;">
        ...
        <h2 style="font-family:'Cinzel',serif; color:var(--vert2); margin-top:4px;">
            Modifier — <?= htmlspecialchars($membre['nom'] . ' ' . $membre['prenom']) ?>
        </h2>
    </div>
            ...
            <form method="POST">
                ...
                <div class="form-grid">
                    ...
                    <div class="champ">
                        ...
                        <input type="text" id="prenom" name="prenom" value="<?= htmlspecialchars($v['prenom'] ?? '') ?>" required>
                    </div>
                </div>
                ...
                <div class="form-grid">
                    <div class="champ">
                        ...
                        <select id="type_membre" name="type_membre" required onchange="toggleRattachement(this.value)">
                            ...
                            <option value="<?= $t ?>" <?= ($v['type_membre'] ?? '') === $t ? 'selected' : '' ?>>
                                <?= ucfirst($t) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="champ">
                        ...
                        <input type="text" id="code_membre" name="code_membre" value="<?= htmlspecialchars($v['code_membre'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div id="section-fivondronana" class="champ">
                    ...
                    <select id="id_fivondronana" name="id_fivondronana">
                        ...
                        <option value="<?= $f['id_fivondronana'] ?>"
                            <?= ($v['id_fivondronana'] ?? '') == $f['id_fivondronana'] ? 'selected' : '' ?>>
                ...
                <div class="form-grid">
                    <div class="champ">
                        ...
                        <input type="date" id="date_naissance" name="date_naissance"
                               value="<?= htmlspecialchars($v['date_naissance'] ?? '') ?>">
                    </div>
                    <div class="champ">
                        <label for="fiantso">Fiantso (surnom scout)</label>
                        <input type="text" id="fiantso" name="fiantso" value="<?= htmlspecialchars($v['fiantso'] ?? '') ?>">
                    </div>
                </div>

                <div class="champ">
                    <label for="adresse">Adresse</label>
                    <input type="text" id="adresse" name="adresse" value="<?= htmlspecialchars($v['adresse'] ?? '') ?>">
                </div>

                <div class="form-grid">
                    <div class="champ">
                        <label for="telephone">Téléphone</label>
                        <input type="text" id="telephone" name="telephone" value="<?= htmlspecialchars($v['telephone'] ?? '') ?>">
                    </div>
                    <div class="champ">
                        <label for="email">Email</label>
                        <input type="email" id="email" name="email" value="<?= htmlspecialchars($v['email'] ?? '') ?>">
                    </div>
                </div>
...
function toggleRattachement(type) {
    document.getElementById('section-branche').style.display =
        ['beazina','mpanabe'].includes(type) ? 'block' : 'none';
    document.getElementById('section-fivondronana').style.display =
        ['kp','mpanohana','ray aman-dreny'].includes(type) ? 'block' : 'none';
}
